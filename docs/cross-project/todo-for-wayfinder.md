...
| Issue | Thema | Status |
|-------|-------|--------|
| [Wayfinder#261](https://github.com/ManuelRingwald/Wayfinder/issues/261) (`from-firefly`) | **ICD 3.7.1 (dokumentarisch, SAFE.4/FR-OPS-009):** CAT065-NOGO (`0x40` in I065/040) wird jetzt tatsächlich gesendet, wenn Fireflys Tracker-Fortschritts-Watchdog anschlägt (> 3 Ausgabeperioden ohne Output-Tick — hängender Kern, FHA H-F1-02). Kein Wire-Bruch (Werte seit 2.3.0 spezifiziert). Wayfinder: nur prüfen (S1), dass ein NOGO-Heartbeat sichtbar als „Dienst degradiert" erscheint und nicht bloß die Staleness-Uhr zurücksetzt. | ⏳ offen |
| [Wayfinder#257](https://github.com/ManuelRingwald/Wayfinder/issues/257) (`from-firefly`) | **Doku-Spiegel zu ADR 0042 (SRV.1, rein dokumentarisch):** Fireflys ADR 0042 schreibt die Arbeitsteilung fest — die ARTAS-**Server-Funktion** (Konsumenten-Verwaltung, Zuschnitt, adressierte Zustellung; klassisch CAT252) ist eine **Verbund-Leistung**: Firefly = Fire-and-Forget-Multicast an alle (konsumenten-blind), Wayfinder = Mandanten/Feeds/Abos + fail-closed AOI/FL-Filter (WF2-21.2) + Auth-WS je Client + Cloud-Fan-out (ADR 0007) + Sensor-Mix je Konsument via eigene Firefly-Instanz (ADR 0012). CAT252 in Firefly bewusst verworfen (Rand-Adapter bliebe möglich). Wayfinder: kurzer Doku-Verweis auf ADR 0042 (Charter §1 o. Verweis-ADR). Kein Code, kein Wire-Bezug. S1–S2. | ⏳ offen |
| [Wayfinder#245](https://github.com/ManuelRingwald/Wayfinder/issues/245) (`from-firefly`) | **CAT062 ICD 3.7.0 (additiv, FR-TRK-048, FPL.2; ADR 0039):** **I062/390** (FRN 21, Compound) trägt die zentrale Flugplan-Korrelation — CSN (#2, 7 Okt. ASCII, Plan-Callsign), DEP (#7)/DST (#8, je 4 Okt. ICAO-Locator); nur bei korreliertem Track, unkorreliert byte-identisch alt; Referenz-Bytes ICD §4.10. Dazu **WS-JSON additiv** aus FPL.1 (`identity_conflict`, `flight_plan`) und die **Kommando-API** für manuelle Korrelation (`POST/DELETE/GET /correlation`, Bearer-Token; Pin stirbt mit TSE). Wayfinder: Decoder FRN 21 + Label (Plan-Callsign, ADEP/ADES, Callsign-Mismatch I062/245 ≠ CSN) + Bedienhandlung gegen die API (Vorstufe EFS-1). Kein Lockstep. S3. | ⏳ offen |
| [Wayfinder#244](https://github.com/ManuelRingwald/Wayfinder/issues/244) (`from-firefly`) | **FPL.0-Abstimmung (ADR 0038, Status „vorgeschlagen"):** Flugplan-Korrelation läuft **zentral im SDPS** (Firefly) — eine Zuordnung für alle Arbeitsplätze, wie bei ARTAS. Vertrag für Wayfinder: Korrelation kommt aus dem Strom (I062/390, FPL.2), manuelle Korrelation ist eine Bedienhandlung gegen Fireflys Kommando-API, Anzeige-Mandantierung bleibt bei Wayfinder. **Vorbedingung für EFS-1** — bitte bestätigen bzw. EFS-Anforderungen (Mindest-Flugplan-Felder fürs Label) einbringen. Regeln: Callsign-first, Squawk nur bei Eindeutigkeit (Weeze-Lektion), nie bei `identity_conflict`, nie Code 1000. | ⏳ Abstimmung offen |
