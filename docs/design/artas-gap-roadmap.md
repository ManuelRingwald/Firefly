...
| | SPEC.2b | **Verfeinerung (aus ADR 0037 ausgewiesen):** Expositions-Buchführung für die Clutter-Karte — je Zelle auch die Scan-Exposition zählen, damit Ereignis-Abwesenheit *belegbar* wird und die Karte λ in sauberen Regionen ehrlich **unter** den Default senken darf (in SPEC.2 bewusst nicht geliefert: Event-only-Schätzer, Floor = Default); dazu Metrik-Ausleitung (`firefly_clutter_cells`, Hotspot-Zähler). Kein eigener Fähigkeits-Zuwachs — Prozente bleiben bei 71 %. | S3 · Sonnet / Fable 5 | **71 %** | ⏳ |
