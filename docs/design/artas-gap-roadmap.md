...
| | VERT.4 | **Tracking-Hälfte:** Constant-Acceleration-Modell in die IMM-Bank (6-D-Zustand x/y/vx/vy/ax/ay) — verbessert die **Prädiktion** in Längsbeschleunigungs-Phasen (Start-/Steigbeschleunigung, Abbremsen im Anflug), die ein CV+CT-IMM als Prozessrauschen behandelt und hinterherläuft; liefert einen **Filterzustand** der Beschleunigung (rausch-/lag-ärmer als die abgeleitete VERT.3-Schätzung) und speist so I062/210 direkt. **Kern-Refactor mit Fusions-Wirkung:** gemischt-dimensionale IMM-Mischung (Zustands-Augmentation/Projektion zwischen 4-D- und 6-D-Modellen) und dimensions-generischer bzw. paralleler 6-D-Pfad durch `kalman`/`gating`/`pda`/`jpda`/`association`/`registration` — **eigener ADR** (die gesamte Fusion ist heute `Matrix4`/`Vector4`-fest). *Zurückgestellt aus VERT.3, weil ein Anzeige-Item den Umbau nicht rechtfertigt; nachzuziehen, wenn die Prädiktions-Qualität in Beschleunigungs-Phasen zählt.* | S5 · Fable 5 / Opus 4.8 | **66,5 %** | ⏳ |
