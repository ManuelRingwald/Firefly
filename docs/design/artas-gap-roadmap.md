...
| | SRV.2 | Laufzeit-Steuerung + Supervision (FR-OPS-008; SNMP-/CMD-Ersatz): **Sensor-Gate** (`SensorGate`, gefiltert ganz vorn in `LiveTracker::ingest` — deaktivierter Sensor erreicht weder Aufzeichnung noch Registrierung noch Tracker) + **Kommando-API** (`GET /sensors` Inventar mit CAT063-Liveness, `POST /sensors/{id}/disable`/`enable` — 422 unbekannt, 409 Standby, idempotent; `GET /status` Supervision-JSON: Rolle/Readiness/Restore/Failover/Tracks/Sensoren/Korrelation/Snapshot/JPDA-Kappe), Auth wie FPL.2 (Bearer-only); Metriken `firefly_sensors_disabled` + `firefly_sensor_disabled_plots_dropped_total`, WARN bei Disable. *Bewusst: Gate flüchtig + pro Instanz (fail-open „mehr Daten" — vergessenes persistiertes Gate wäre schlimmer); CAT063 bleibt Quell-Wahrheit (kein stiller ICD-Semantik-Wechsel); kein Config-Reload (neue Quellen = Neustart/Orchestrator); Polled-Adapter pollen weiter.* | S3 · Fable 5 (Empf. Sonnet) | **97,5 %** | ✅ 2026-07-16 |
...
| **AP-MON · Monitoring** (nach 100 %, Betreiber-Entscheid 2026-07-16) | MON.1 | Monitoring-Paket auf **Bestands-Tooling** (kein Eigenbau): Alertmanager-Regeln `monitoring/prometheus/alerts.yaml` (~10 begründete Alarme in 3 Schweregraden: Feed tot, Snapshot veraltet, Sensor-Ausfall, Back-Pressure-Verlust, Failover/Split-Brain, JPDA-Kappe u. a.) + **Runbook-Tabelle** (Alarm → Bedeutung → Handgriff inkl. `/sensors`-Kommandos), Grafana-Dashboard-**Neubau** nach Blöcken (Feed/Quellen/Tracker/HA/Korrelation — Bestand hat nur 5 SDPS-006-Panels), optional ServiceMonitor + Rules im Helm-Chart, Log-Aggregations-Rezept (Loki/ELK) in TECHNICAL. *Offene Design-Frage, kein gesetzter Baustein: externer Feed-Watchdog (Konsumenten-Sicht) — nur relevant für Betrieb ohne Wayfinder und **nur nach eigenem Design-Konzept + Freigabe** (Betreiber-Vorgabe 2026-07-16); Sandbox validiert nur Syntax, Grafana-Sichtprüfung = Betreiber-Schritt.* | S2–S3 · Sonnet | — (nach 100 %) | ⏳ |
