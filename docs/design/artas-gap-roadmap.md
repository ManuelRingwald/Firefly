...
| | HA.2b | Split-Brain-Schutz + Failover-Observability: Demotion (aktiver Main, der einen fremden aktiven Heartbeat derselben Identität sieht, tritt in Standby zurück — deterministischer Tie-Breaker), Metriken `firefly_role`/`firefly_failovers_total`/Heartbeat-Alter, dokumentierte Partitions-Grenzen (ADR-0041-Nachweis) | S4 · Fable 5 | **85 %** | ⏳ |
