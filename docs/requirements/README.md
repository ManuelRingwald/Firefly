...
| NFR-OPS-001 | Einfache Vorführbarkeit: Der Tracker/eine Demo lässt sich lokal mit einem einzigen Befehl und ohne Programmierkenntnisse starten (Showcase/Präsentation). | in Arbeit (Server startet per `cargo run -p firefly-server`) | Eingebaute Demo-Szene (`scene.rs`), 12-Factor-Start ohne Flags. Politur (Demo-Szene + sichtbarer „Verzug") in Häppchen 3.5. |
