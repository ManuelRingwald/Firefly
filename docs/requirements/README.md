...
| FR-IO-004 | Ein CAT062-Datenblock kann **dekodiert** werden — die Umkehrung von FR-IO-003: FSPEC parsen, die enthaltenen FRNs in UAP-Reihenfolge lesen und jedes Item in seinen Ursprungswert zurückrechnen (Data Source, Time of Track, WGS84-Position, system-stereografisches X/Y in Metern, Geschwindigkeit, Track-Nummer, Status, Update-Alter, Genauigkeit, optional Identität). Grundlage für eine Empfänger-/Recorder-Seite (Häppchen D), die das gesendete Signal unabhängig verifiziert. I062/100 wird hier nur als X/Y in Metern zurückgewonnen; die Rückprojektion nach WGS84 (`StereographicProjection::unproject`, FR-GEO-004) folgt in einem Folge-Häppchen. | fertig (D.1) | `firefly-asterix`: `fspec::parse_inverts_fspec`, `fspec::parse_of_empty_input_consumes_nothing`, `cat062::decode_inverts_encode_for_a_plain_track`, `cat062::decode_recovers_identity_when_present`, `cat062::decode_recovers_status_for_tentative_coasting_track`, `cat062::decode_handles_multiple_records`, `cat062::decode_handles_empty_block`, `cat062::decode_rejects_malformed_input` |
| FR-NET-002 | Der CAT062-Strom wird zusätzlich als **UDP-Multicast** versendet (ADR 0006-Nachtrag, ED-109A-Stil): je Scan ein CAT062-Datenblock an eine Multicast-Gruppe; Konsumenten (ASD, EFS, Recorder) hören unabhängig mit, der Sender kennt sie nicht (Entkopplung, ADR 0003). Eigene Crate `firefly-multicast`, 12-Factor-Konfiguration (`FIREFLY_CAT062_*`: Gruppe, Port, SAC/SIC, Referenzpunkt), per Default **aus**. Die Zustellung wird am Ausgabe-Rand nach Datenzeit getaktet, ohne den datenzeit-getriebenen Kern zu berühren. Beide Ausgabe-Adapter (Web/JSON, CAT062/Multicast) übersetzen denselben `SystemTrack`-Strom (`Player::scans`) unabhängig voneinander (M3.X.4). | verifiziert | `firefly-multicast`: `sender::sends_one_cat062_block_per_scan`, `sender::empty_scan_list_sends_nothing`, `pacing::*`, `config::*`; `firefly-player`: `scans_and_frames_share_the_same_scan_times` |
